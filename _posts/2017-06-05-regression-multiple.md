...
Time for the most commonly used and abused data analysis technique: Regression (multiple regression). In many cases, it is common practice to run a regression of the data just for explanation; an invaluable tool that ties in subjective information in an objective manner.
...
While this analysis will not end the debate on climate change, it is certainly our best bet for inference. Our hypothesis will check to see if (H<sub>o</sub>) there is no significant difference in greenhouse gas effect is true. We will observe 5 different types of greenhouse gasses (co2, methane, n2o, sf6, hcfc) and compare these individual values.

### Data and EDA

Data is collected from scraping the NASA website (using 'XML'; 
Citation of the data will be at the bottom!). Most of the code was borrowed by Scott Grimshaw (BYU) and, afterwards, polished by myself. 

If you want to follow along, download the datasets in this [repository](https://github.com/tykiww/projectpage/tree/master/datasets/Climate%20data) or source the code below to begin.

```r
source("https://raw.githubusercontent.com/tykiww/tykiww.github.io/master/gasses.R")
```

Just some background on how the data was collected for clarity.
...
Let's begin by installing our libraries. `plotly` will be used to visualize. It comes in extremely handy to detect outliers and leverage points. The other new libraries that appear here are the `car` and `outliers` package. The car stands for "Companion to Applied Regression". Many of the functions run will be tested for outliers and checking irregularities (collinearity and such).
...
Let's move on to downloading the data. Click [here](https://github.com/tykiww/projectpage/tree/master/datasets/Climate%20data) to find the datasets for each item. These are each in csv text files you can just open up anywhere.

We will have a total of 6 dataframes to load. 1 response variable for temperature, and 5 explanatory variables for each greenhouse gas. Temperature data was first fit to match the rest of the other sets where the end goal is to have temp, month, and year. There were some weird quirks with the tables where month 13 was the cumulative of the year, so those were removed tp create new months.
...
Just to check our normality assumptions, we will use the shapiro-wilk test rather than the KS test used [previously](https://tykiww.github.io/2017-02-05-SLR-Plotly/). `1 - output` should give us the alternative hypothesis that there is a difference. Plots will also be made to assess the data.
...
pvals[3:8] # p-values from Shapiro Wilk test not so normal.
...
We'll start by refitting the model without each high collinearity value one at a time and comparing the model estimation power. *The comparisons will be skipped due to time constraints*, but it is rather simple. All we need to do is to take out the variable that has the highest collinearity and check to see if the model predicts better without that particular greenhouse gas. If that doesn't work, shuffle around which variables are most important and impactful. Transforming the data may also be an option as well. Also check to see that the percentage of error estimation (R^2) is still high for our model. There needs to be a good optimization of useful information and high prediction. 
...
Unfortunately, after a while of manipulating data, n2o, sf6, and hcfc needed to be removed. This was originally due to the high collinearity of sf6 and n2o. The model prediction R^2 was a lot better keeping co2 in the data and removing hcfc. co2 was kept as the parameter of interest as co2, water vapor, and methane were the most abundant greenhouse gasses in the atmosphere. Though there are ways to mitigate collinearity (using generalized least squares), however, we will provide the best 'fit' model. The new model will look like this:
...
Not bad. Many points may be influential in the x direction, but they seem to be strongly supporting the prediction of the data. The R^2 value increased a little with co2 a little less significant. Yet we are still good to go. After we remove those values, we can check out the outliers. 
...
It seems like we have a model that predicts about 40 percent of the variance. Our beta hat values also seem to significantly show that these estimates do indeed reject the null hypothesis of no effect. For what information we currently have, we may surmise that greenhouse gasses do have an effect on global warming. Now, whether global warming is a seasonal trend or not will have to be predicted with a time series, but for what we know now it is safe to say that the gasses we put into the air do have a role to play.

The whole process can oftentimes be arduous and long. It really is up to how well you hope to predict and use the information. I like to see how careful I can be and think about the most relevant information for our needs. Of course, with larger sets and larger data, it becomes very difficult to fit a huge linear regression (it might overfit). For cases with large columns, maybe other big-data techniques such as XGBoost, RF, or Lasso regressions may be more useful.

Good luck in your future data endeavors.
